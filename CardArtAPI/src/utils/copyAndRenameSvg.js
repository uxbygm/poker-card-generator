...
const sourceFile = path.join(__dirname, '../assets/traditional_1/ace_of_hearts.svg');
const destinationDir = path.join(__dirname, '../assets/traditional_1');

cardTypes.forEach(cardType => {
    const destinationFile = path.join(destinationDir, `${cardType}.svg`);
    fs.copyFileSync(sourceFile, destinationFile);
    console.log(`Copied to ${destinationFile}`);
});